...
mkdir -p qa audit/reports audit/screenshots/qa docs/script-runs _backups

if ! command -v node >/dev/null 2>&1; then
  echo "Node.js is required for rendered QA, axe, and Lighthouse."
...
echo "Installing local QA dependencies..."
npm install --save-dev playwright axe-core lighthouse >/dev/null

echo "Installing Playwright Chromium..."
npx playwright install chromium >/dev/null

cat > qa/sofiati_full_quality_system.mjs <<'JS'
#!/usr/bin/env node
...
chmod +x qa/sofiati_full_quality_system.mjs

echo "Running comprehensive QA with safe fixes, rendered browser checks, screenshots, axe, and selected Lighthouse..."
node qa/sofiati_full_quality_system.mjs --all --fix-safe | tee docs/script-runs/full-comprehensive-qa.txt

echo ""
echo "Open the report:"
echo "audit/reports/MASTER-COMPREHENSIVE-QA.md"
