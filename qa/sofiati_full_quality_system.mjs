...
function ensureSectionSignatures(text, concept, pageName) {
  ...
  return text.replace(/<section\b[^>]*>/gi, (tag) => {
    ...
    return tag.replace(
      />$/,
      ` data-layout-signature="${concept}-${page}-section-${String(count).padStart(2, "0")}">`,
    );
  });
}
...
function ensureSeoBasics(text, concept, pageName) {
  ...
  const desc =
    bodyDesc ||
    `${label} information for Franciele Sofiati, with evaluation-led guidance, responsible expectations and a clear path toward consultation.`;
  const canonical =
    bodyCanonical || `https://www.sofiati.com/concepts/${concept}/${pageName}`;
...
function fixStatusBanner(file) {
  ...
  if (!/public-utility-name/.test(next)) {
    next = next.replace(
      /(<[^>]+class=["'][^"']*public-utility[^"']*["'][^>]*>)/i,
      `$1\n<span class="public-utility-name">Franciele Sofiati</span>`,
    );
  }

  if (
    !/data-lang-switch=["']en["']/i.test(next) ||
    !/data-lang-switch=["']pt["']/i.test(next)
  ) {
  ...
  next = next.replace(
    /(<button\b[^>]*data-lang-switch=["']en["'][^>]*)(>)/i,
    (m, before, end) =>
      /aria-pressed=/i.test(before) ? m : `${before} aria-pressed="true"${end}`,
  );
  next = next.replace(
    /(<button\b[^>]*data-lang-switch=["']pt["'][^>]*)(>)/i,
    (m, before, end) =>
      /aria-pressed=/i.test(before)
        ? m
        : `${before} aria-pressed="false"${end}`,
  );
...
function fixMobileMenu(file) {
  ...
  let next = text.replace(
    /\s*<div class=["'][^"']*public-language-menu[^"']*["'][\s\S]*?<\/div>/gi,
    "",
  );

  if (!/id=["']mobile-menu["']/i.test(next)) {
    next = next.replace(
      /<([a-z0-9]+)([^>]*class=["'][^"']*mobile[^"']*["'][^>]*)>/i,
      `<$1 id="mobile-menu"$2>`,
    );
  }
...
function fixJs(file) {
  ...
  let next = replaceBlock(
    text,
    LANGUAGE_RUNTIME_START,
    LANGUAGE_RUNTIME_END,
    LANGUAGE_RUNTIME,
  );
...
function auditInventory() {
  ...
  if (dirs.length !== 50) {
    addIssue(
      ...
      "GLOBAL",
      CONCEPTS_DIR,
      `Expected 50 concept folders, found ${dirs.length}.`,
      "Restore or create missing concept folders.",
    );
  }
    ...
    for (const partial of REQUIRED_PARTIALS) {
      ...
      if (!fs.existsSync(file)) {
        addIssue(
          ...
          concept,
          file,
          `Missing required partial ${partial}.`,
          "Restore or create this partial.",
        );
      }
    }
    ...
    for (const asset of ["css/style.css", "js/main.js", "js/partials.js"]) {
      ...
      if (!fs.existsSync(file)) {
        addIssue(
          ...
          concept,
          file,
          `Missing core asset ${asset}.`,
          "Restore required CSS/JS.",
        );
      }
    }
    ...
    for (const doc of [
      ...
      if (!fs.existsSync(file)) {
        addIssue(
          "inventory",
          "medium",
          "missing-planning-doc",
          concept,
          file,
          `Missing planning document ${doc}.`,
          "Create concept planning doc.",
        );
      }
    }
  }
    ...
    if (fs.existsSync(status)) {
      ...
      if (!/public-utility/i.test(text))
        addIssue(
          ...
          concept,
          status,
          "Status banner missing public utility class.",
          "Use .public-utility.status-banner.",
        );
      if (!/public-utility-name/i.test(text))
        addIssue(
          "partials",
          "high",
          "utility-name",
          concept,
          status,
          "Utility name class missing.",
          "Add .public-utility-name.",
        );
      if (!/Franciele Sofiati/.test(text))
        addIssue(
          "partials",
          "high",
          "canonical-name",
          ...
          "Use Franciele Sofiati consistently.",
        );
      if (
        (text.match(/data-lang-switch=["']en["']/gi) || []).length !== 1 ||
        (text.match(/data-lang-switch=["']pt["']/gi) || []).length !== 1
      ) {
        addIssue(
          "partials",
          "high",
          "language-switcher-count",
          concept,
          status,
          "Utility should have exactly one EN and one PT language control.",
          "Keep EN/PT in utility bar.",
        );
      }
      if (!/aria-pressed/i.test(text))
        addIssue(
          "partials",
          "medium",
          "language-aria",
          concept,
          status,
          "Language controls missing aria-pressed.",
          "Add aria-pressed.",
        );
    }
    ...
    if (fs.existsSync(header)) {
      ...
      if (!/public-header/i.test(text))
        addIssue(
          ...
          concept,
          header,
          "Header missing .public-header.",
          "Add required header wrapper.",
        );
      if (!/data-menu-toggle/i.test(text))
        addIssue(
          "partials",
          "critical",
          "menu-toggle",
          concept,
          header,
          "Header missing data-menu-toggle.",
          "Add mobile menu trigger.",
        );
      if (!/header-consultation/i.test(text))
        addIssue(
          "partials",
          "high",
          "header-cta",
          concept,
          header,
          "Header missing consultation CTA class.",
          "Add .header-consultation.",
        );
    }

    if (fs.existsSync(mobile)) {
      const text = read(mobile);
      if (!/id=["']mobile-menu["']/i.test(text))
        addIssue(
          "partials",
          "critical",
          "mobile-menu-id",
          concept,
          mobile,
          "Mobile menu missing #mobile-menu.",
          "Add id.",
        );
      if (!/data-menu-close/i.test(text))
        addIssue(
          "partials",
          "critical",
          "mobile-close",
          concept,
          mobile,
          "Mobile menu missing close control.",
          "Add data-menu-close.",
        );
      if (!/public-mobile-primary/i.test(text))
        addIssue(
          "partials",
          "high",
          ...
          concept,
          mobile,
          "Mobile primary link container missing.",
          "Add .public-mobile-primary.",
        );
      if (!/public-mobile-cta/i.test(text))
        addIssue(
          "partials",
          "high",
          "mobile-cta",
          ...
          mobile,
          "Mobile CTA missing.",
          "Add .public-mobile-cta.",
        );
      if (/public-language-menu/i.test(text))
        addIssue(
          "partials",
          "medium",
          "duplicate-menu-language",
          concept,
          mobile,
          "Language switcher found inside mobile menu.",
          "Keep language switcher in utility bar only.",
        );
    }
      ...
      for (const label of PRIMARY_NAV) {
        if (!text.includes(label)) {
          addIssue(
            ...
            concept,
            nav,
            `Navigation missing ${label}.`,
            "Keep required primary nav labels.",
          );
        }
      }
    }
    ...
    for (const file of pageFiles(dir)) {
      ...
      if (body && !/data-page=/i.test(body))
        addIssue(
          ...
          concept,
          file,
          "Body missing data-page.",
          "Add data-page metadata.",
        );
      ...
      for (const bad of BAD_TEXT) {
        if (lower.includes(bad))
          addIssue(
            ...
            concept,
            file,
            `Suspicious placeholder text found: ${bad}.`,
            "Replace with final client-facing copy.",
          );
      }

      const sections = [
        ...text.matchAll(/<section\b[^>]*>[\s\S]*?<\/section>/gi),
      ];
      if (!["404.html", "sitemap.html"].includes(page) && sections.length < 3) {
        addIssue(
          ...
          concept,
          file,
          `Only ${sections.length} sections found.`,
          "Review page storytelling depth manually.",
        );
      }
      ...
      sections.forEach((match, index) => {
        ...
        if (!/<h[2-6]\b/i.test(raw))
          addIssue(
            ...
            concept,
            file,
            `Section ${index + 1} has no H2-H6 heading.`,
            "Add semantic heading.",
          );
        if (!/data-layout-signature/i.test(sectionTag))
          addIssue(
            "pages",
            "low",
            "missing-layout-signature",
            concept,
            file,
            `Section ${index + 1} missing data-layout-signature.`,
...
function auditSeoSchema() {
    ...
    for (const file of pageFiles(dir)) {
      ...
      if (!title)
        ...
        addIssue(
          ...
          concept,
          file,
          `Title length ${title.length} outside ideal range.`,
          "Aim around 30-75 characters.",
        );
      ...
      else if (desc.length < 70 || desc.length > 180)
        addIssue(
          "seo",
          "medium",
          "description-length",
          concept,
          file,
          `Description length ${desc.length} outside ideal range.`,
          "Aim around 70-180 characters.",
        );
      ...
      if (!/<link\s+rel=["']canonical["']/i.test(text))
        addIssue(
          ...
          file,
          "Missing canonical.",
          "Add absolute canonical URL.",
        );
      if (!/property=["']og:title["']/i.test(text))
        addIssue(
          "seo",
          "low",
          "missing-og-title",
      ...
      if (!jsonLdBlocks.length && !fs.existsSync(schemaPartial)) {
        addIssue(
          ...
          concept,
          file,
          "No JSON-LD or schema partial detected.",
          "Add schema partial or inline JSON-LD.",
        );
      }
    }
  }
  ...
  if (!fs.existsSync(path.join(ROOT, "sitemap.xml")))
    addIssue(
      "seo",
      "high",
      "missing-root-sitemap",
      "GLOBAL",
      "sitemap.xml",
      "Root sitemap.xml missing.",
...
function auditAccessibilityStatic() {
  for (const dir of conceptDirs()) {
    ...
    if (!/:focus-visible|:focus/i.test(cssText))
      addIssue(
        "accessibility",
        "medium",
        "missing-focus-style",
        concept,
        css,
        "No focus/focus-visible style found.",
    ...
    for (const file of pageFiles(dir)) {
      ...
      for (const img of text.match(/<img\b[^>]*>/gi) || []) {
        if (!/\balt=["'][^"']+["']/i.test(img))
          addIssue(
            ...
            concept,
            file,
            "Image missing non-empty alt.",
            "Add meaningful alt or alt='' for decorative.",
          );
        if (/\balt=["']\s*(image|photo|picture|graphic)\s*["']/i.test(img))
          addIssue(
            "accessibility",
            "medium",
            "weak-alt",
      ...
      for (const button of text.match(/<button\b[^>]*>[\s\S]*?<\/button>/gi) ||
        ...
        if (!clean && !/aria-label=/i.test(button))
          addIssue(
            "accessibility",
            "high",
            "button-name",
  ...
  for (const dir of conceptDirs()) {
    ...
    for (const file of pageFiles(dir)) {
      ...
      for (const match of text.matchAll(/\b(href|src)=["']([^"']+)["']/gi)) {
        ...
        if (
          !value ||
          /^(#|mailto:|tel:|http:\/\/|https:\/\/|whatsapp:|javascript:)/i.test(
            value,
          )
        )
          continue;
        ...
        if (!fs.existsSync(target)) {
          addIssue(
            ...
            file,
            `Broken ${attr}: ${value}`,
            "Fix path or restore missing file.",
          );
        }
      }
    }
...
function auditCssJs() {
  ...
  const nodeAvailable = !!spawnSync("node", ["--version"], { encoding: "utf8" })
    .stdout;
  ...
  for (const dir of conceptDirs()) {
    ...
    if (fs.existsSync(css)) {
      ...
      if (importantCount > 20)
        addIssue(
          ...
          concept,
          css,
          `${importantCount} !important rules found.`,
          "Reduce specificity problems.",
        );
      ...
      if (/body\s*{[^}]*overflow-x\s*:\s*hidden/is.test(text))
        addIssue(
          "css",
          "low",
          ...
          concept,
          css,
          "body overflow-x hidden may mask layout bugs.",
          "Fix overflowing elements instead.",
        );
      if (fs.statSync(css).size > 180000)
        addIssue(
          "performance",
          "medium",
          "large-css",
          concept,
          css,
          `CSS is ${Math.round(fs.statSync(css).size / 1024)}KB.`,
          "Reduce repeated/unused CSS.",
        );
    }
    ...
    for (const file of [js, partialsJs]) {
      ...
      if (path.basename(file) === "main.js" && !/data-lang-switch/i.test(text))
        addIssue(
          "js",
          "high",
          ...
          "Add language state JS.",
        );
      if (
        path.basename(file) === "main.js" &&
        !/data-menu-toggle|mobile-menu/i.test(text)
      )
        addIssue(
          "js",
          "high",
          "missing-menu-runtime",
          concept,
          file,
          "Mobile menu runtime missing.",
          "Add menu open/close JS.",
        );
      if (fs.statSync(file).size > 120000)
        addIssue(
          "performance",
          "medium",
          "large-js",
          concept,
          file,
          `JS is ${Math.round(fs.statSync(file).size / 1024)}KB.`,
          "Reduce JS.",
        );
  ...
  for (const [h, names] of cssHashes) {
    if (names.length > 1)
      addIssue(
        ...
        `Identical CSS shared by ${names.length} concepts: ${names.slice(0, 20).join(", ")}.`,
        "Make concepts structurally different.",
      );
  }
  for (const [h, names] of headerHashes) {
    if (names.length > 1)
      addIssue(
        ...
        `Identical header shared by ${names.length} concepts: ${names.slice(0, 20).join(", ")}.`,
        "Vary header structure/layout.",
      );
  }
  for (const [h, names] of mobileHashes) {
    if (names.length > 1)
      addIssue(
        ...
        `Identical mobile menu shared by ${names.length} concepts: ${names.slice(0, 20).join(", ")}.`,
        "Vary mobile menu design/build.",
      );
  }
  for (const [h, names] of footerHashes) {
    if (names.length > 1)
      addIssue(
        ...
        `Identical footer shared by ${names.length} concepts: ${names.slice(0, 20).join(", ")}.`,
        "Vary footer structure/layout.",
      );
  }
}
...
function auditPerformanceStatic() {
  for (const dir of conceptDirs()) {
    ...
    for (const file of pageFiles(dir)) {
      const text = read(file);
      for (const img of text.match(/<img\b[^>]*>/gi) || []) {
        if (!/\sloading=/i.test(img) && !/hero/i.test(img))
          addIssue(
            "performance",
            "low",
            ...
            concept,
            file,
            "Non-hero image missing loading='lazy'.",
            "Add loading='lazy'.",
          );
        if (!/\sdecoding=/i.test(img))
          addIssue(
            "performance",
            "low",
...
function auditUxStatic() {
  for (const dir of conceptDirs()) {
    ...
    if (fs.existsSync(header)) {
      const text = read(header);
      if (!/Consultation|Consulta/i.test(text))
        addIssue(
          "ux-ui",
          "high",
          "weak-primary-cta",
          concept,
          header,
          "Header does not visibly include consultation CTA.",
          "Make consultation the primary action.",
        );
      if ((text.match(/<a\b/gi) || []).length > 12)
        addIssue(
          "ux-ui",
          "medium",
          "crowded-header",
          concept,
          header,
          "Header may be crowded.",
          "Keep desktop nav clear.",
        );
    }

    if (fs.existsSync(mobile)) {
      const text = read(mobile);
      if ((text.match(/<a\b/gi) || []).length < 6)
        addIssue(
          "ux-ui",
          "high",
          "thin-mobile-menu",
          concept,
          mobile,
          "Mobile menu has too few links.",
          "Include primary nav links.",
        );
      if (!/aria-hidden/i.test(text))
        addIssue(
          "ux-ui",
          "medium",
          "mobile-menu-aria-hidden",
          concept,
          mobile,
          "Mobile menu missing aria-hidden.",
          "Add accessible open/closed state.",
        );
    }
      ...
      for (const label of PRIMARY_NAV) {
        if (!text.includes(label))
          addIssue(
            ...
            nav,
            `Missing ${label}.`,
            "Keep required navigation labels.",
          );
      }
    }
  }
          ...
          try {
              ...
              async ({ primaryNav, isMobile }) => {
                ...
                const navLinks = Array.from(
                  document.querySelectorAll(
                    ".public-header .desktop-nav a, .desktop-nav a",
                  ),
                ).filter(visible);
                const navLabels = navLinks.map((a) =>
                  (a.textContent || "").trim(),
                );
                const navRows = new Set(
                  navLinks.map((a) =>
                ...
                if (pt && en) {
                  ...
                  await wait(180);
                  const ptWorks =
                    pt.getAttribute("aria-pressed") === "true" &&
                ...
                return {
                  ...
                  navLabels,
                  navRows,
                  missingNav: primaryNav.filter(
                    (label) => !navLabels.includes(label),
                  ),
                  languageWorks,
                  mobileMenuWorks,
                };
            ...
            if (metrics.pageOverflow > 2)
              addIssue(
                ...
                concept,
                `${pageName} ${vp.name}`,
                `Horizontal overflow ${Math.round(metrics.pageOverflow)}px.`,
                "Fix layout width.",
              );
            if (!metrics.headerVisible)
              addIssue(
                "rendered",
                "critical",
                "header-not-visible",
                concept,
                `${pageName} ${vp.name}`,
                "Header not visible after render.",
                "Fix partial loading/header CSS.",
              );
            if (!metrics.footerVisible)
              addIssue(
                ...
                concept,
                `${pageName} ${vp.name}`,
                "Footer not visible after render.",
                "Fix footer partial/rendering.",
              );
            if (!metrics.utilityVisible)
              addIssue(
                "rendered",
                "high",
                "utility-not-visible",
                concept,
                `${pageName} ${vp.name}`,
                "Utility/status bar not visible.",
                "Fix status-banner partial/rendering.",
              );
            ...
            if (metrics.h1Count !== 1)
              addIssue(
                ...
                concept,
                `${pageName} ${vp.name}`,
                `Rendered H1 count is ${metrics.h1Count}.`,
                "Keep one rendered H1.",
              );
            if (!metrics.languageWorks)
              addIssue(
                "rendered",
                "high",
                "rendered-language-toggle",
                concept,
                `${pageName} ${vp.name}`,
                "Rendered EN/PT toggle failed.",
                "Fix language runtime/partials.",
              );
            if (!metrics.mobileMenuWorks)
              addIssue(
                "rendered",
                "critical",
                ...
                concept,
                `${pageName} ${vp.name}`,
                "Rendered mobile menu failed.",
                "Fix menu runtime/partials.",
              );
            if (!vp.isMobile && metrics.navRows > 1)
              addIssue(
                "rendered",
                "medium",
                "desktop-nav-wrap",
                concept,
                `${pageName} ${vp.name}`,
                `Desktop nav wraps into ${metrics.navRows} rows.`,
                "Adjust desktop nav spacing.",
              );
            if (!vp.isMobile && metrics.missingNav.length)
              addIssue(
                "rendered",
                "high",
                "rendered-missing-nav",
                concept,
                `${pageName} ${vp.name}`,
                `Missing nav labels: ${metrics.missingNav.join(", ")}.`,
                "Fix nav partial.",
              );
            ...
            for (const violation of axe.violations
              ...
              addIssue(
                ...
                concept,
                `${pageName} ${vp.name}`,
                `${violation.help} (${violation.nodes.length} nodes).`,
                violation.helpUrl,
              );
            }
    ...
    for (const target of lighthouseTargets()) {
      ...
      if (result.status !== 0) {
        addIssue(
          ...
          target.concept,
          target.page,
          result.stderr || result.stdout || "Lighthouse failed.",
          "Check local Chrome/Lighthouse setup.",
        );
        continue;
      }
    ...
    try {
      ...
      if (!response.ok) {
        addIssue(
          ...
          "Check deployed URL or try later.",
        );
        continue;
      }
      ...
      for (const [key, cat] of Object.entries(cats)) {
        ...
        if (score < 90) {
          addIssue(
            ...
            url,
            `${cat.title}: ${score}/100.`,
            "Open PageSpeed report and optimize.",
          );
        }
      }
    } catch (error) {
...
function writeReports(backupPath = "") {
  ...
  const lines = [];
  lines.push(
    "# Sofiati Comprehensive QA, Rendered, Lighthouse and Refactor Report",
  );
  lines.push("");
  ...
  lines.push("");
  if (changes.length) {
    for (const item of [...new Set(changes)].sort())
      lines.push(`- \`${item}\``);
  } else {
  ...
  lines.push("| --- | ---: |");
  for (const [aspect, count] of groupCount(issues, "aspect"))
    lines.push(`| ${aspect} | ${count} |`);
  ...
  lines.push("| --- | ---: |");
  for (const [kind, count] of groupCount(issues, "kind"))
    lines.push(`| ${kind} | ${count} |`);
  ...
  for (const item of autoFixable) {
    lines.push(
      `| ${item.severity} | ${item.aspect} | ${item.kind} | ${item.concept} | \`${item.file}\` | ${item.detail.replaceAll("|", "\\|")} | ${item.fix.replaceAll("|", "\\|")} |`,
    );
  }

  lines.push("");
  lines.push(
    "## Manual design, content, uniqueness and performance-refactor issues",
  );
  lines.push("");
  lines.push(
    "| Severity | Aspect | Kind | Concept | File | Detail | Recommendation |",
  );
  lines.push("| --- | --- | --- | --- | --- | --- | --- |");
