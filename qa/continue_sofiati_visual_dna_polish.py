#!/usr/bin/env python3
"""Resumable Sofiati premium visual DNA polish runner.

This wrapper keeps the broad repair resumable from the terminal. It delegates
safe HTML/CSS/report updates to qa/restore_sofiati_premium_visual_dna.mjs and
uses docs/script-runs/visual-dna-polish-progress.json as the ledger.
"""
...
NODE_SCRIPT = ROOT / "qa" / "restore_sofiati_premium_visual_dna.mjs"
PROGRESS_PATH = ROOT / "docs" / "script-runs" / "visual-dna-polish-progress.json"
...
def incomplete_concepts(progress: dict[str, dict]) -> list[str]:
    ...
    return [
        ...
        if known.get(concept_id, {}).get("status") != "complete"
    ]
