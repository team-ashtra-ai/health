...
(() => {
  ...
  "menuMode": "studio-card",
  "headerMode": "route-map-header",
  "interactionTone": "whatsapp-strip",
  "destinationWords": [
    "service",
    "route",
    "system",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 09-radiance`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c09-header");
    if (!header) return;
