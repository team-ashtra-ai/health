...
(() => {
  ...
  "menuMode": "botanical-panel",
  "headerMode": "layered-magazine-header",
  "interactionTone": "text-link",
  "destinationWords": [
    "polished",
    "brand",
    "signature",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 48-signature`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c48-header");
    if (!header) return;
