...
(() => {
  ...
  "menuMode": "magazine-drawer",
  "headerMode": "quiet-portrait-header",
  "interactionTone": "dark-anchor",
  "destinationWords": [
    "flagship",
    "premium",
    "luxury",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 50-sovereign`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c50-header");
    if (!header) return;
