...
(() => {
  ...
  "menuMode": "clinical-list",
  "headerMode": "clinical-grid-header",
  "interactionTone": "soft-pill",
  "destinationWords": [
    "selective",
    "editorial",
    "curated",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 46-curate`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c46-header");
    if (!header) return;
