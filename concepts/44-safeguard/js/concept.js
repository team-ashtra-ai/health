...
(() => {
  ...
  "menuMode": "quiet-canvas",
  "headerMode": "deep-salon-header",
  "interactionTone": "single-rule",
  "destinationWords": [
    "trust",
    "safety",
    "first",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 44-safeguard`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c44-header");
    if (!header) return;
