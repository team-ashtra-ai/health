...
(() => {
  ...
  "menuMode": "warm-sheet",
  "headerMode": "minimal-rule-header",
  "interactionTone": "inline-route",
  "destinationWords": [
    "guided",
    "pathway",
    "site",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 43-meridian`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c43-header");
    if (!header) return;
