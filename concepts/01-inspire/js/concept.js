...
(() => {
  ...
  "menuMode": "dark-sheet",
  "headerMode": "editorial-index-header",
  "interactionTone": "split-quiet",
  "destinationWords": [
    "dark",
    "premium",
    "editorial",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 01-inspire`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c01-header");
    if (!header) return;
