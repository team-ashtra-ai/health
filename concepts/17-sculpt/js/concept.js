...
(() => {
  ...
  "menuMode": "route-index",
  "headerMode": "warm-conversation-header",
  "interactionTone": "minimal-arrow",
  "destinationWords": [
    "geometric",
    "premium",
    "site",
  ...
  const fetchPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(name, fetch(`partials/${name}.html`, { cache: "no-store" }).then((response) => {
        if (!response.ok) throw new Error(`Missing partial ${name} for 17-sculpt`);
  ...
  const wireHeaderState = () => {
    const header = document.querySelector(".c17-header");
    if (!header) return;
