(() => {
  const concept = {
    folder: "45-silhouette",
    number: "45",
    name: "Silhouette",
    archetype: "clinical skincare drops",
  ...
  const pageMeta = () => ({
    ...
    canonical:
      document.body.dataset.canonical ||
      `${concept.domain}/concepts/${concept.folder}/${pageFile()}`,
  });
  const getPartial = async (name) => {
    if (!cache.has(name)) {
      cache.set(
        name,
        fetch(partialPath(name), { cache: "no-store" }).then((response) => {
  ...
  const interpolate = (html) => {
    ...
    return html
      ...
      .replaceAll(
        "{{SCHEMA_JSON}}",
        JSON.stringify(buildSchema(meta), null, 2),
      );
  };
  ...
  const injectHeadPartial = async (name) => {
    ...
    [
      ...document.head.querySelectorAll(`[data-dynamic-partial="${name}"]`),
    ].forEach((node) => node.remove());
    if (name === "head") {
      document.head
        .querySelectorAll(
          'title,meta[name="description"],link[rel="canonical"],meta[property^="og:"],meta[name="theme-color"],link[rel="icon"],link[rel="apple-touch-icon"]',
        )
        .forEach((node) => node.remove());
    }
  ...
  const injectMounts = async (name) => {
    const html = await getPartial(name);
    document
      .querySelectorAll(`[data-partial-mount="${name}"]`)
      .forEach((mount) => {
  ...
  const applyNavigation = async () => {
    ...
    document.querySelectorAll("[data-navigation-slot]").forEach((slot) => {
      ...
      const template =
        source.content.querySelector(`[data-navigation-template="${mode}"]`) ||
        source.content.querySelector('[data-navigation-template="primary"]');
        ...
        itemListElement: [
          {
            ...
            position: 1,
            name: "Home",
            item: `${concept.domain}/concepts/${concept.folder}/index.html`,
          },
    ...
    if (meta.page === "faq") {
      base.push({
        ...
        mainEntity: [
          {
            "@type": "Question",
            name: "Do results vary?",
            acceptedAnswer: {
              "@type": "Answer",
              text: "Results vary according to individual characteristics, evaluation, indication, protocol, sessions and aftercare.",
            },
          },
          {
            "@type": "Question",
            name: "Can I choose a laser directly?",
            acceptedAnswer: {
              "@type": "Answer",
              text: "Laser suitability should be discussed through professional evaluation before treatment selection.",
            },
          },
        ],
      });
  ...
  const load = async () => {
    ...
    await Promise.all([
      injectMounts("floating-whatsapp"),
      injectMounts("back-to-top"),
    ]);
    await applyNavigation();
    document.body.dataset.partialsReady = "true";
    document.dispatchEvent(
      new CustomEvent("sofiati:partials-ready", {
        detail: { concept: concept.folder },
      }),
    );
  };
